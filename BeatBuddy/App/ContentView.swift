...
class GameLogic2: NSObject, ObservableObject, AVAudioPlayerDelegate, CountdownCircleViewDelegate {
...
protocol CountdownCircleViewDelegate2: AnyObject {
    func didMissBeat(_ countdownCircleView: CountdownCircleView2)
}

class CountdownCircleView2: UIView {
    ...
    weak var delegate: CountdownCircleViewDelegate2?
    ...
    var body: some View {
        NavigationStack(path: $pathStore.path) {
            MenuView(viewModel: DependencyInjection.init().provideMenuViewModel())
                .navigationDestination(for: RouterPath.self) { viewPath in viewPath.view }
        }
